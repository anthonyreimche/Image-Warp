...
export function WarpPanel() {
  ...
  if (!warpActive) {
    return h(
      ...
      h("button", { onClick: () => void clearWarp(), style: BTN({}) }, "Clear warp"),
    );
  }
  ...
  return h(
    ...
    h(
      "div",
      { style: { fontSize: "10.5px", color: "var(--color-accent)", lineHeight: 1.4 } },
    ...
    h("div", { style: { height: "1px", background: "var(--color-border-subtle)", margin: "2px 0" } }),

    slider("Size", size, 0.02, 0.6, 0.14, (v) => store.getState().setSize(v)),
    slider("Density", density, 0, 1, 0.5, (v) => store.getState().setDensity(v)),
    slider("Pressure", pressure, 0, 1, 1, (v) => store.getState().setPressure(v)),
    slider("Rate", rate, 0, 1, 0.5, (v) => store.getState().setRate(v)),
    slider("Hardness", hardness, 0, 1, 0.5, (v) => store.getState().setHardness(v)),
